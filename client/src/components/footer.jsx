...
export default function Footer() {
  return (
    <footer>
      <div
        className="w-full h-64 flex justify-between pt-4 pb-4 flex-col items-center"
        style={{
      ...
      >
        <div className="flex justify-between items-start w-7/12">
          <div className="column text-white flex flex-col">
            ...
            <span className="text-sm  font-Play">
              Do you need any kind of help please contact with us.
            </span>
          </div>
          <form className="flex">
            <input
              className="[outline:none] bg-[transparent] relative rounded-xl border-solid border-gray-500 border text-white  hover:cursor-text  w-80 p-3"
              type="email"
        ...
        <span className="text-cc-color text-sm">
          Copyright @2024 CodeBid - All rights reserved{" "}
        </span>
      </div>
    </footer>
  );
...
function CustomSocialCircle({ src, alt }) {
  return (
    <div className="w-[50px] h-[50px] rounded-full bg-gray box-border overflow-hidden border-[1px] border-solid border-rose-400 bg-[#181b26]">
      <img src={src} alt={alt} />
    </div>
  );
}
